...
jQuery(function($) {
    ...
    let trToObj = function($tr, ruleset) {
        let result = {};
        $($tr).find('td').each(function(idx) {

            let arr = $(this).text().split('(');
            let text = $.trim(기호제거(arr[0]));
    ...
    const parseHeader2 = function($표) {
        ...
        headerInfo = modifyRuleset($표.find('thead tr:eq(1) th'), headerInfo, partyInfo, 4);
    ...
    const parse대통령개표단위 = function() {
        ...
        $표.find('tbody tr:gt(0)').each(function() {
            ...
            */
            ...
            const is사전투표 = subsNull(구분1[투표구.읍면동명], subsNull(구분2[투표구.투표구명], false));
            ...
            for (const 소속 of Object.keys(정당코드)) {
                총계_대상[소속] = 투표구[소속] + subsNull(총계_대상[소속], 0);
            }
        });
        ...
        const 지역목록 = JSON.parse(subsNull(localStorage.getItem(`대통령세부_목록_${필터.도시}`), '{}'));
        지역목록[필터.구시군] = date;
    ...
    const parse대통령세부 = function() {
        ...
        localStorage.setItem('대통령지역_' + subsNull(필터.도시, '전체'), JSON.stringify(총계));
        if (필터.도시 !== '') {
    ...
    const display대통령개표단위 = function() {
        ...
        for (const 지역 of Object.keys(지역코드)) {
            const 도시목록 = JSON.parse(subsNull(localStorage.getItem(`대통령세부_목록_${지역}`), '{}'));
            ...
            for (const 구시군 of Object.keys(도시목록)) {
                ...
                for (const [세부계, 세부] of target) {
                    ...
                    for (const 정당 of Object.keys(정당코드)) {
                        const value = 세부[정당];
                        세부계[정당] = value + subsNull(세부계[정당], 0);
            ...
            $표.append(toTr(세부_사전_계, headerInfo, `${지역}(사전)`));
            $표.append(toTr(세부_본_계, headerInfo, `${지역}(본)`));
        }

        const 비율 = {};
        for (const [코드, value] of Object.entries(총계)) {
            비율[코드] = (value / 총계['계'] * 100).toFixed(2) + '%';
        }
        $표.append(toTr(총계, headerInfo, '계'));
    ...
    const display대통령 = function() {
        ...
        $표.append('<tr><td></td></tr>');
        for (const 지역 of Object.keys(지역코드)) {
            const 도시정보 = JSON.parse(subsNull(localStorage.getItem(`대통령지역_${지역}`), '{}'));
            if (!도시정보) {
                continue;
            }
            $표.append(toTr(도시정보, headerInfo, 지역));
            for (const [정당, value] of Object.entries(도시정보)) {
                if (!(정당 in 정당코드)) {
                    continue;
                }
